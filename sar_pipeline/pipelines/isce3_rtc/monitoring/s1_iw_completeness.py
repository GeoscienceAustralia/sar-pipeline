...
def query_cdse_for_bursts_in_period(
    ...
    if isinstance(geometry, MultiPolygon):
        ...
        logging.info(
            "WARNING : Provided geometry is a MultiPolygon. A separate query will be made for each "
            f"Polygon within the MultiPolygon. Simplify if possible and ensure Polygons are not overlapping. "
            f"N polygons = {n_polygons}"
        )
    else:
        n_polygons = 1

    logging.info(
        f"Separating query into {chunk_query_minutes*n_polygons} "
        f"minute chunks with {query_overlap_seconds} second overlap "
        f"(odata API limits to 1000 responses per request)"
    )
    logging.info(
        f"{len(query_dt_chunks)} chunked queries will be made in parallel with {max_workers} workers"
    )
...
if __name__ == "__main__":
    ...
    start_dt = datetime(2024, 12, 14, 0, 0, 0)  # ant test
    ...
    geometry = load_geojson_as_shape(shape_url).simplify(tolerance=0.1)
    ...
    # Get the burst products missing from S3
    burst_products_not_in_s3 = {}
    burst_products_in_s3 = {}

    for burst_id, azimuth_time in tqdm(
        expected_burst_product_dict.keys(), desc="Searching S3 bucket for products"
    ):
    ...
    if n_missing_from_s3:
        logging.info(
            f"{n_missing_from_s3} of {n_expected_products} expected burst "
            f"products are missing from the S3 bucket : {s3_bucket}"
        )
    else:
        logging.info(
            f"All {n_expected_products} products exist in the S3 bucket : {s3_bucket}"
        )
    if n_scenes_to_process:
        logging.info(
            f"{n_scenes_to_process} of {n_expected_scenes} expected scenes have missing burst products"
        )
    else:
        logging.info(f"All {n_expected_scenes} expected scenes have been processed.")
    ...
    stac_start_dt = _format_dt(start_dt - buffer)
    stac_end_dt = _format_dt(end_dt + buffer)
    logging.info(
        f"Searching STAC API with buffered time range : {stac_start_dt} to {stac_end_dt}"
    )
    stac_items = stac_client.search(
        collections=collections_to_search,
        datetime=f"{stac_start_dt}/{stac_end_dt}",
        intersects=shape(geometry),
    )
    n_stac_matches = stac_items.matched()
    ...
    for i in range(0, 1):
        for burst_product in tqdm(
            ...
            # plogging.info(burst_product)
            odc_id = burst_product["id"]
            burst_id = burst_product["properties"]["sarard:burst_id"]
            azimuth_time = burst_product["properties"]["datetime"]
            scene_id = burst_product["properties"]["sarard:scene_id"]
            polarisation_list = burst_product["properties"]["sar:polarizations"]
            ...
            if (burst_id, azimuth_time) not in expected_burst_product_dict:
                # additional product not in the list from the CDSE that we
                # have picked up with the expanded time buffer
                continue

            # check if the product is in the indexed dict already therefore a duplicate
            ...
            # we are dealing with a duplicate item
                ...
                # create a list of the duplicate products
                ...
                if created_time > _existing_created_time:
                    ...
                    duplicate_products_to_archive[(burst_id, azimuth_time)].append(
                        burst_products_indexed[(burst_id, azimuth_time)].copy()
                    )
    ...
    # make the dictionary of products not indexed as from expected and indexed dict
    burst_products_not_indexed = {
        k: v
    ...
    if n_missing_from_odc:
        logging.info(
            f"{n_missing_from_odc} of {n_expected_products} expected burst "
            f"products are not indexed and are missing from the ODC"
        )
        if n_existing_burst_products_to_index:
            f"{n_existing_burst_products_to_index} existing products need to be indexed into the ODC."
    else:
        logging.info(f"All {n_expected_products} are indexed in the ODC")

    logging.info(
        f"{n_duplicate_products_to_archive} duplicate products have been found in the ODC and should be archived"
    )
    ...
    if identify_missing_linked_static_layers:
        if n_scenes_to_process == 0:
            ...
            cdse_stac_catalog = "https://stac.dataspace.copernicus.eu/v1/"
            stac_client = pystac_client.Client.open(cdse_stac_catalog)
            configure_s3_access(cloud_defaults=True, aws_unsigned=True)
            logging.info(f"Connecting to STAC API catalog : {cdse_stac_catalog}")
            logging.info(
                f"Searching STAC API with buffered time range : {stac_start_dt} to {stac_end_dt}"
            )
            # stac query STAC for items with slight buffer in times
            ...
            # we need to use the scenes to determine which DEM is used for the missing static layers
            for scene in tqdm(
                scenes_to_process,
                ...
                for burst_id, azimuth_time in scene_bursts:
                    ...
                    # logging.info(
                    #     f"searching for static lay3rs in s3 folder : {expected_static_layer_s3_product_folder}"
                    # )
    ...
    # 5. Identify missing from ODC index -> MISSING_FROM_ODC
    # 6. Identify missing from S3 -> MISSING_FROM_AWS_S3_BUCKET
    # 7. Identify those in S3 missing from index (need re-indexing) -> PROCESSED_NEEDS_INDEXING
    # 8. Identify those missing from S3 (need reprocessing) -> UNPROCESSED_NEEDS_PROCESSING
    # 9. Identify those indexed and missing from S3 (need reprocessing) -> ORPHANED_NEEDS_PROCESSING
    # 10. Identify Duplicates and those that need Archiving -> DUPLICATE_NEEDS_ARCHIVING
            ...
            for k, v in obj.items():
                ...
                if isinstance(k, tuple):
                    key = (
                        "("
                        + ", ".join(
                            repr(_format_dt(t) if isinstance(t, datetime) else t)
                            for t in k
                        )
                        + ")"
                    )
    ...
    completeness_json = make_json_safe(completeness_dict)

    with open(completeness_report_filepath, "w") as f:
